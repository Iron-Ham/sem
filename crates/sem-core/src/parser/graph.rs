...
"#;
    ...
    fn test_same_line_container_and_child_refs_use_byte_spans() {
        ...
        let content = "export function helper() { return 1; }\n\
             export function other() { return 2; }\n\
             export class Service { static { helper(); } method() { return other(); } }\n";
        write_file(root, "app.ts", content);

        let (graph, entities) = EntityGraph::build(root, &["app.ts".into()], &registry);
        let class_id = "app.ts::class::Service";
        let method_id = "app.ts::class::Service::method";
        let helper_id = "app.ts::function::helper";
        ...
        let entity_map: HashMap<String, EntityInfo> = entities
            ...
            .collect();
        let tree = registry
            .extract_entities_with_tree("app.ts", content)
            .and_then(|(_, tree)| tree)
            .expect("TypeScript parser should produce a tree");
        let scope_result = scope_resolve::resolve_with_scopes(
            root,
            &["app.ts".into()],
            &entities,
            &entity_map,
            Some(vec![("app.ts".into(), content.to_string(), tree)]),
        );
        assert!(
            scope_result.edges.iter().any(|(from, to, ref_type)| {
                from == class_id && to == helper_id && *ref_type == RefType::Calls
            }),
            "scope resolver should keep the same-line static-block helper call. Edges: {:?}",
            scope_result.edges
        );
        assert!(
            scope_result.edges.iter().any(|(from, to, ref_type)| {
                from == method_id && to == other_id && *ref_type == RefType::Calls
            }),
            "scope resolver should keep the same-line method-body other call. Edges: {:?}",
            scope_result.edges
        );
    }
