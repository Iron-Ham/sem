        ...
        maybe_par_iter!(parsed_files)
            .filter_map(|(file_path, content, tree)| {
                ...
                let descendant_ranges_by_entity =
                    build_descendant_ranges_by_entity(&file_entities, entity_map, &entity_spans);
...
fn log_scope_bindings(
    ...
    start_row: usize,
    end_row: usize,
    descendant_ranges_by_entity: &HashMap<String, Vec<DescendantRange>>,
) {
...
fn build_descendant_ranges_by_entity(
    ...
    entity_map: &HashMap<String, EntityInfo>,
    entity_spans: &HashMap<&str, SourceSpan>,
) -> HashMap<String, Vec<DescendantRange>> {
    let mut ranges_by_entity: HashMap<String, Vec<DescendantRange>> = HashMap::new();
    ...
    for entity in sorted_entities {
        ...
        let child_range = DescendantRange {
            start_row: entity.start_line.saturating_sub(1),
            end_row: entity.end_line,
            span: entity_spans.get(entity.id.as_str()).copied(),
        };
    ...
    for ranges in ranges_by_entity.values_mut() {
        ranges.sort_unstable_by_key(|range| {
            (
                range.start_row,
                range.end_row,
                range.span.map(|span| (span.start_byte, span.end_byte)),
            )
        });
        ranges.dedup();
    }
...
fn row_belongs_to_descendant(
    ...
    descendant_ranges_by_entity
        .get(entity_id)
        .map_or(false, |ranges| {
            let eligible = ranges.partition_point(|range| range.start_row <= row);
            ranges[..eligible]
                .iter()
                .rev()
...
fn ref_belongs_to_descendant(
    ...
    descendant_ranges_by_entity
        .get(entity_id)
        .map_or(false, |ranges| {
            let eligible = ranges.partition_point(|range| range.start_row <= ast_ref.row);
            ranges[..eligible].iter().rev().any(|range| {
                if ast_ref.row < range.start_row || ast_ref.row >= range.end_row {
                    return false;
                }
                range.span.map_or(true, |span| {
                    ast_ref.start_byte >= span.start_byte && ast_ref.end_byte <= span.end_byte
                })
            })
        })
}
